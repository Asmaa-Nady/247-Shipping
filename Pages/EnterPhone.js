...
export default class EnterPhone extends Component {
  ...
  submit() {
    ...
    if (formData.phone == "") {
      this.refs.toast.show("Please Write Your Phone", DURATION.LENGTH_LONG);
      this.setState({ backgroundColor: "#FFA5A5" });
    } else {
      var url = "http://172.16.4.183/api/index.php/User/forgetPassword";
      fetch(url, {
        method: "POST",
        ...
        .then(res => res.json())
        .then(
          NavigationService.navigate("PasswordActivation", {
  ...
  render() {
    return (
      ...
      >
        <WhiteHeader header="Forget Password" />
        {/* //////////////////////////// */}
        <Toast
          ref="toast"
        ...
        >
          ...
          <Text style={{ fontSize: Dimensions.get("window").width * 0.04 }}>
            Note : This Phone Will get Activation Code
          </Text>
          <Button
            rounded
            style={{
